...
public class UserControllerTest {
	...
	public void should_loginUser() {
		...
		try {
			...
			RequestBuilder requestBuilder = MockMvcRequestBuilders.post("/users").accept(MediaType.APPLICATION_JSON)
					.content(mockUserJson).contentType(MediaType.APPLICATION_JSON);
			// get result
			MvcResult result = mockmvc.perform(requestBuilder).andReturn();
			MockHttpServletResponse mockResponse = result.getResponse();
			// check result
	...
	public void should_updateUser() {
		...
		try {
			...
			ObjectMapper mapper = new ObjectMapper();
			String mockUserJson = mapper.writeValueAsString(mockUser);
			RequestBuilder requestBuilder = MockMvcRequestBuilders.put("/users").accept(MediaType.APPLICATION_JSON)
					.content(mockUserJson).contentType(MediaType.APPLICATION_JSON).sessionAttr("logged in", true);
			// get result
			MvcResult result = mockmvc.perform(requestBuilder).andReturn();
			MockHttpServletResponse mockResponse = result.getResponse();
			// check result
	...
	public void should_notUpdateUser() {
		...
		try {
			...
			String mockUserJson = mapper.writeValueAsString(mockUser);
			RequestBuilder requestBuilder = MockMvcRequestBuilders.put("/users").accept(MediaType.APPLICATION_JSON)
					.content(mockUserJson).contentType(MediaType.APPLICATION_JSON).sessionAttr("logged in", false);
			// get result
			MvcResult result = mockmvc.perform(requestBuilder).andReturn();
			MockHttpServletResponse mockResponse = result.getResponse();
			// check result
